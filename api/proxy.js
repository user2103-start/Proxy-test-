// ============================================================
// api/proxy.js - COMPLETE WITH TEST FUNCTIONALITY + VIDEO BACKUP
// ============================================================
...
// 5. VIDEO/PDF SOURCE DETAILS - ORIGINAL
if (action === "video") {  
...
// ============================================================
// ✅ VIDEO BACKUP API - For when main video URL is not found
// ============================================================
if (action === "video-backup") {
  ...
  console.log("📡 VIDEO BACKUP: Getting video for:", content_id);

  try {
    // Try to get video from main API first
    const response = await fetch(`${NT}/course/content-details?content_id=${content_id}&course_id=${course_id}`, {
      method: "GET",
      headers: headers
    });
    ...
    const videoUrl = data?.data?.video_url || data?.data?.stream_url || data?.video_url || data?.stream_url;
    
    if (videoUrl) {
      // Video URL found - return it
      ...
      // Video URL not found - return friendly message
      return res.status(200).json({
        success: true,
        found: false,
        message: "🔴 Stream URL not found at the moment",
        friendly_message: "⏳ Wait for a few hours or minutes! The video URL will be available soon. Please try again later.",
    ...
    // Error fetching video - return user-friendly message
    console.error("❌ Video backup error:", error);
    return res.status(200).json({
      success: true,
      found: false,
      message: "⚠️ Unable to fetch video URL at this time",
      friendly_message: "⏳ Hold tight! We're trying to load the video. Please wait for a few minutes and try again. The URL will be available soon! 🚀",
      content_id: content_id,
      course_id: course_id,
      error: error.message,
      retry_after: 300,
...
// ============================================================
// ✅ ALTERNATIVE VIDEO BACKUP - Using different endpoint
// ============================================================
if (action === "video-alt") {
  const { content_id, course_id } = req.query;
  ...
  console.log("📡 VIDEO ALT: Fetching from alternative source:", content_id);

  try {
    // Try alternative endpoint
    const response = await fetch(`${NT}/course/content-source?content_id=${content_id}&course_id=${course_id}`, {
      method: "GET",
      headers: headers
    });

    const data = await response.json();
    const videoUrl = data?.data?.source_url || data?.data?.video_url || data?.source_url || data?.video_url;
    ...
    if (videoUrl) {
      return res.status(200).json({
        ...
        found: true,
        source: "alternative",
        message: "Video URL found from alternative source"
      });
    } else {
      return res.status(200).json({
        success: true,
        ...
        friendly_message: "🎬 The video is being processed. Please check back in a few minutes. We appreciate your patience! 🙏",
        content_id: content_id,
        course_id: course_id,
        retry_after: 180,
        timestamp: new Date().toISOString()
    ...
    return res.status(200).json({
      ...
      friendly_message: "⏳ The video URL is currently unavailable. Please wait for a few minutes or try again later. The content will be ready soon! ✨",
      content_id: content_id,
      course_id: course_id,
      retry_after: 300,
      timestamp: new Date().toISOString()
