// api/proxy.js
// Nexttoppers API — Complete login system + all endpoints with strict frontend mappings

const NT = "https://course.nexttoppers.com";
const AUTH = "https://auth.nexttoppers.com";
const TEST = "https://test.nexttoppers.com"; // Domain for test handling
const APP_ID = "1772100600";
const DEVICE_ID = "ae2fa506-85ca-418d-a449-ec5068dc6665";

// Fallback token (used if user not logged in)
const FALLBACK_TOKEN = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjozMjQ1MDMzLCJhcHBfaWQiOiIxNzcyMTAwNjAwIiwiZGV2aWNlX2lkIjoiYWUyZmE1MDYtODVjYS00MThkLWE0NDktZWM1MDY4ZGM2NjY1IiwicGxhdGZvcm0iOiIzIiwidXNlcl90eXBlIjoxLCJpYXQiOjE3Nzg0OTQ5NjcsImV4cCI6MTcheck0OTY3fQ.40g-NIb1n2f8oA7PPIqTgD3Y1zDrsQCpCxBajwpMaJY";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // action=content — Folders + Videos + PDF Filtering (Supports core IDs: 151, 152, 107)
    if (action === "content") {
    ...
    // action=video — Video details + stream URL / VDC ID fallback
    if (action === "video") {
    ...
    // action=live — Live classes filtering mapping exactly via structural constraints
    if (action === "live") {
      ...
      // Dynamic Filter Check: file_type === 2 && video_type === 3 && is_live === 1
    ...
    // ══════════════════════════════════════════
    // TEST APIs (JEE / NEET / ENGINE INTEGRATION)
    // ══════════════════════════════════════════
    ...
    // action=test-info — Test instructions (Accepts layout entity_id mapped dynamically)
    if (action === "test-info") {
      const test_id = req.query.test_id || body.test_id || req.query.entity_id || body.entity_id;
      if (!test_id) return res.status(400).json({ error: "test_id or entity_id required" });
    ...
    // action=test-data — Test questions (Accepts layout entity_id mapped dynamically)
    if (action === "test-data") {
      const test_id = req.query.test_id || body.test_id || req.query.entity_id || body.entity_id;
      if (!test_id) return res.status(400).json({ error: "test_id or entity_id required" });
